...
async def test_pipeline_anti_bot_degraded_succeeds_without_playwright(
    monkeypatch,
    success_product_html,
):
    """Página tem anti-bot overlay mas expõe JSON-LD → classificador retorna
    SUCCESS(anti_bot_degraded) → pipeline extrai dados sem acionar Playwright.

    Telemetria esperada: anti_bot_detected=True, fallback_taken=False, layer=curl_cffi.
    """
    # HTML com challenge cloudflare + JSON-LD de produto (sinal de produto presente)
    degraded_html = (
        "<html><head>"
        '<script type="application/ld+json">{"@type":"Product","name":"Notebook Integrado",'
        '"offers":{"@type":"Offer","price":"1999.90","priceCurrency":"BRL"}}</script>'
    ...
    fake_pw_pool = MagicMock()
    fake_pw_pool.is_ready = False  # Playwright offline; não deve ser chamado
    ...
    # Classifier real: HTML tem cloudflare + application/ld+json → anti_bot_degraded

    outcome = await run_pipeline("https://example.com/product/degraded")

    assert outcome.status == "success"
    assert outcome.payload is not None
    assert outcome.payload["name"] == "Notebook Integrado"

    # Telemetria deve refletir sucesso degradado via HTTP
    assert outcome.context.data.get("layer_used") == "curl_cffi"
    assert outcome.context.data.get("fallback_taken") is False
    assert outcome.context.data.get("anti_bot_detected") is True
    assert outcome.context.data.get("anti_bot_bypassed") is False
    assert outcome.context.data.get("classification_reason") == "anti_bot_degraded"


_USEFUL_JSON_LD_HTML = (
    "<html><body>"
    '<script type="application/ld+json">'
    '{"@type":"Product","name":"Notebook Browser","offers":{"price":"1999.00","priceCurrency":"BRL"}}'
    "</script></body></html>"
)


async def test_pipeline_late_browser_escalation_resolves_when_http_html_not_parseable(
    monkeypatch,
):
    """HTTP retorna HTML sem dado útil; LateBrowserEscalationStep obtém HTML via browser e extrai."""
    _empty_html = "<html><body><p>sem dados</p></body></html>"
    ...
    fake_pw_pool = MagicMock()
    fake_pw_pool.is_ready = True
    fake_pw_pool.fetch_html = AsyncMock(return_value=_USEFUL_JSON_LD_HTML)

    monkeypatch.setattr("market_scraper.services.pipeline_steps.robots.is_allowed", fake_is_allowed)
    monkeypatch.setattr("market_scraper.services.fetch_decision_gate.download_html", fake_download)
    monkeypatch.setattr("market_scraper.services.fetch_decision_gate.adaptive_rate_limiter", _fake_rate_limiter())
    monkeypatch.setattr("market_scraper.services.fetch_decision_gate._classifier", _fake_classifier_success())
    monkeypatch.setattr("market_scraper.services.pipeline_steps.playwright_pool", fake_pw_pool)

    outcome = await run_pipeline("https://example.com/product/late-browser")

    assert outcome.status == "success"
    assert outcome.payload is not None
    assert outcome.payload["name"] == "Notebook Browser"
    assert outcome.context.data.get("fallback_taken") is True
    assert outcome.context.data.get("layer_used") == "playwright"
    assert outcome.context.data.get("browser_escalated_for_utility") is True
