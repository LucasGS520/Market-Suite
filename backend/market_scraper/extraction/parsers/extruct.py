""" Utilitários de parsing de dados estruturados via Extruct

O módulo extrai metadados estruturados (JSON-LD, OpenGraph) presentes
no HTML de páginas de produtos usando ``extruct`` como motor principal.
...
def parse_with_extruct(html: str, url: str | None = None) -> dict[str, str] | None:
    ...
    if not name or not price:
        ...
        if isinstance(opengraph, list):
            ...
            if not price:
                price = _sanitize_price(_as_text(
                    og_map.get("product:price:amount") or og_map.get("og:price:amount")
                ))
