...
de itens processados. O loop principal em ``manager.py`` aguarda o resultado
via ``celery.result.AsyncResult.get()`` em vez de usar canvas callbacks.
...
def _dispatch_collect_task(
    *,
    ...
    Retorna o task_id gerado pelo Celery, ou None se o envio falhar.
    Sem canvas callbacks — o loop em manager.py aguarda via AsyncResult.get().
    """
    from market_alert.schemas.schemas_collection_payload import validate_payload as _vp
    try:
        typed_payload = _vp(payload)
        task_id = CollectionEnqueuer()._send(typed_payload)
        logger.info(
...
def _collect_group(
    *,
    ...
    O task_id do monitorado é incluído na decisão para que o loop em manager.py
    possa aguardar via AsyncResult.get() e chamar finalize_processing_requeue diretamente.
    """
    ...
    task_id = _dispatch_collect_task(
    ...
    outcome = "enqueued_partial" if competitor_failures else "enqueued"
    return CollectDispatchDecision(
        outcome=outcome,
        next_check_at=None,
        should_requeue=False,
...
def _requeue_monitored(
    *,
    ...
    if queue.reenqueue_after_collection(monitored.id, now, source="continuous_worker"):
        logger.warning(
