...
def _patch_monitored(monkeypatch: pytest.MonkeyPatch) -> tuple[Mock, Mock, Mock]:
    ...
    fetch_mock = Mock(return_value=fetch_result)
    close_mock = Mock()
    monkeypatch.setattr(monitored.ScraperClient, "fetch", fetch_mock)
    monkeypatch.setattr(monitored.ScraperClient, "close", close_mock)
...
def _patch_competitor(monkeypatch: pytest.MonkeyPatch) -> tuple[Mock, Mock, Mock]:
    ...
    monkeypatch.setattr(competitor.ScraperClient, "close", close_mock)
...
def test_scrape_monitored_sanitiza_metadados(
...
def test_scrape_monitored_propagates_last_modified(
    monkeypatch: pytest.MonkeyPatch,
    fake_monitored_payload: MonitoredProductCreateScraping,
) -> None:
    ...
    monitored.scrape_monitored_product(
        db=Mock(spec=Session),
        url=fake_monitored_payload.product_url,
        user_id=uuid4(),
        payload=fake_monitored_payload,
...
def test_scrape_monitored_handles_not_modified(monkeypatch: pytest.MonkeyPatch, fake_monitored_payload: MonitoredProductCreateScraping) -> None:
...
def test_scrape_competitor_handles_not_modified(monkeypatch: pytest.MonkeyPatch, fake_competitor_payload: CompetitorProductCreateScraping) -> None:
...
def test_scrape_competitor_reutiliza_url_canonica(
    monkeypatch: pytest.MonkeyPatch,
    ...
    fetch_mock = Mock(return_value=ScraperFetchResult(status_code=304, payload=None, headers={}))
    close_mock = Mock()
    monkeypatch.setattr(competitor.ScraperClient, "fetch", fetch_mock)
    monkeypatch.setattr(competitor.ScraperClient, "close", close_mock)
...
def test_scrape_competitor_sanitiza_campos(
    monkeypatch: pytest.MonkeyPatch,
    fake_competitor_payload: CompetitorProductCreateScraping,
) -> None:
    ...
    fetch_mock = Mock(return_value=response)
    close_mock = Mock()
    monkeypatch.setattr(competitor.ScraperClient, "fetch", fetch_mock)
    monkeypatch.setattr(competitor.ScraperClient, "close", close_mock)
