...
def test_price_target_rule_uses_value_and_percent():
    ...
    alert = {"type": "price_below_monitored", "price": 8, "pct_below_monitored": 20}
    assert alert_matches_rule(alert, rule)
    assert alert["pct_below_threshold"] == Decimal("20.00")

    alert = {"type": "price_below_monitored", "price": 12, "pct_below_monitored": 20}
    assert not alert_matches_rule(alert, rule)

    alert = {"type": "price_below_monitored", "price": 9, "pct_below_monitored": 3}
    assert not alert_matches_rule(alert, rule)
...
def test_rule_with_threshold_value_filters_alerts():
    ...
    assert alert_matches_rule({"type": "price_below_monitored", "price": Decimal("4.00")}, rule)
    assert not alert_matches_rule({"type": "price_below_monitored", "price": Decimal("6.00")}, rule)
