...
- **Collector (`tasks.collector_product_task.collect_product_task`)**: processa uma URL por vez (monitorado ou concorrente), tenta obter lock no Redis e retorna um `ScrapeResult` padronizado (`success`, `not_modified`, `no_result`, `error`) contendo `http_status`, `price_changed`/`availability_changed` e `error_code` quando aplicável. Após coleta bem-sucedida, atualiza automaticamente o campo `next_check_at` do produto.
- **Recheck Scheduler (`tasks.recheck_scheduler_task.schedule_rechecks`)**: executado periodicamente pelo Beat, varre produtos com `next_check_at` vencido e enfileira tasks `collect_product_task` na fila `scraping`. O controle de concorrência é feito exclusivamente via locks Redis aplicados pelo collector.
- **Comparação (`tasks.compare_prices_tasks.compare_prices_task`)**: idempotente e leve; disparada automaticamente pelo collector quando detecta mudanças de preço ou disponibilidade, ou acionada manualmente para recalcular histórico/comparativos.
- **Política de locks**: o collector usa `acquire_product_lock` com TTL configurável via `PRODUCT_LOCK_TTL_SECONDS`. Não há mais flags de controle no banco de dados (`checking_in_progress` foi removida).
- **Contratos de desfecho**: quando o lock não é adquirido o collector retorna `no_result` (mantendo métrica de lock skipped) para preservar o contrato enxuto; rechecagens sem mudança (`not_modified`) atualizam `next_check_at` mas não geram novo `PriceHistory`.
...
- **Orquestração de scraping**: use sempre a task central `market_alert.tasks.collector_product_task.collect_product_task` e o serviço `orchestrator/collector_service_orchestrator.py` para enfileirar monitorados e concorrentes. As rechecagens periódicas são agendadas pelo Beat via `tasks.recheck_scheduler_task.schedule_rechecks`, que enfileira coletas na fila `scraping` usando a mesma task `collect_product_task`. O controle de concorrência é feito exclusivamente via locks Redis.
