...
## Remoção de Métricas e Observabilidade (Prometheus, Grafana, Loki)

**Contexto**: O sistema de métricas Prometheus foi completamente desabilitado para simplificar a arquitetura e reduzir overhead operacional.

**Implementação técnica**:
- **Toggle `ENABLE_METRICS`**: variável de ambiente com valor padrão `0` (desabilitado). Configure `ENABLE_METRICS=1` para reabilitar (requer reinstalação de dependências).
- **Stubs no-op**: criado `backend/shared/metrics_noop.py` com implementações vazias de `Counter`, `Histogram`, `Gauge` e `Info` que mantêm a mesma interface mas não executam operações.
- **Imports condicionais**: todos os módulos de métricas (`backend/shared/metrics/*.py`) agora verificam `ENABLE_METRICS` e importam os stubs quando desabilitado.
- **Compatibilidade de código**: imports existentes de métricas continuam funcionando mas são no-op. Não há necessidade de remover chamadas a `.inc()`, `.observe()`, etc.
- **Dependências removidas**: `prometheus_client` e `prometheus-fastapi-instrumentator` foram removidos do `requirements.txt`.
- **Serviços removidos**: Prometheus, Grafana, Loki, Promtail, Node Exporter e cAdvisor foram removidos do `docker-compose.yml`.
- **Backups**: configurações originais foram preservadas em `metrics_backup/` e `monitoring_backup/` para referência futura.

**Impacto operacional**:
- **Endpoints `/metrics`**: continuam existindo mas retornam resposta vazia ou minimal quando métricas estão desabilitadas.
- **Performance**: redução de overhead em coletas, processamento e armazenamento de séries temporais.
- **Logs estruturados**: continuam ativos via `structlog` para depuração e análise.
- **Reabilitação**: para restaurar métricas, defina `ENABLE_METRICS=1`, restaure dependências do backup e reconfigure serviços de monitoramento.

**Diretrizes para agentes**:
- Não remova imports ou chamadas de métricas existentes; elas são no-op e mantêm legibilidade do código.
- Ao adicionar novos fluxos, evite adicionar métricas a menos que `ENABLE_METRICS` esteja ativo no ambiente.
- Para depuração, prefira logs estruturados com `structlog` em vez de métricas.
...
- **Estrutura e estilo**: siga o padrão existente do repositório; não introduza linters/formatadores novos sem alinhamento. Evite `print`; utilize `structlog` para logs estruturados.
- **Testes**: crie ou ajuste testes com `pytest`. Execute `pytest -q` nos módulos afetados antes de concluir alterações.
- **Métricas desabilitadas**: o sistema de métricas Prometheus foi desabilitado por padrão (`ENABLE_METRICS=0`). Os imports continuam funcionando mas são no-op via stubs. Não remova imports de métricas, apenas evite adicionar novos. Para reabilitar, configure `ENABLE_METRICS=1`.
