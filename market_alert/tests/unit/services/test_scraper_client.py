...
async def test_fetch_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("market_alert.services.scraper_client.httpx.AsyncClient", lambda *a, **k: _DummyAsyncClient(responses))
...
async def test_fetch_raises_after_retries(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("market_alert.services.scraper_client.asyncio.sleep", _fake_sleep)
    monkeypatch.setattr("market_alert.services.scraper_client.settings", SimpleNamespace(
