...
from market_alert.tasks.alert_tasks import send_alert_task, send_notification_task, dispatch_price_alert_task
...
def test_notification_task_success(monkeypatch):
    ...
    monkeypatch.setattr("market_alert.tasks.alert_tasks.SessionLocal", lambda: fake_db)
    mid = "123e4567-e89b-12d3-a456-426614174000"
    monitored = SimpleNamespace(id=mid, user_id="u1", name_identification="prod")
    monkeypatch.setattr(
        "market_alert.tasks.alert_tasks.get_monitored_product_by_id",
...
def test_send_alert_task_success(monkeypatch):
    ...
    monkeypatch.setattr("market_alert.tasks.alert_tasks.SessionLocal", lambda: DummyDB())
