...
- **GET `/comparisons/{monitored_id}`**: aceita apenas `limit` (padrão 10) e retorna lista simples de comparações sem paginação ou metadados. [Fonte: `market_alert/routes/routes_comparisons.py`, linhas 33-56.]
- **GET `/comparisons/{monitored_id}/summary`**: usa `PriceComparisonSummaryResponse` com `json_encoders` que convertem `Decimal` em `float`, produzindo números no JSON. [Fonte: `market_alert/routes/routes_comparisons.py`, linhas 58-81; `market_alert/schemas/schemas_comparisons.py`, linhas 33-72.]
