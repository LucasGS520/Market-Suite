...
> Quando habilitar o Cloudscraper? A flag deve permanecer ``true`` em produção. Considere ativar manualmente apenas para domínios problemáticos (preencher `SCRAPER_CLOUDSCRAPER_DOMAINS`) quando a métrica `scraper_http_client_error_total` exibir picos de 4xx e o fallback reduzir a taxa de falhas.
