...
```bash
celery -A market_alert.core.celery_app:celery_app worker --loglevel=debug --pool=threads --concurrency=4
...
## Estrutura
- `market_alert/` – código da aplicação FastAPI, serviços e tarefas Celery
...
Essa função carrega o produto monitorado e os seus concorrentes e utiliza `compare_prices` (módulo `market_alert/utils/comparator.py`) para gerar o resumo de preços.
...
O pacote [`market_alert/notifications`](market_alert/notifications) centraliza todos os canais de envio e a classe `NotificationManager`, responsável por orquestrar
as mensagens. O gerente utiliza [`get_notification_manager`](market_alert/notifications/manager.py) para instanciar os canais configurados e verificar
...
#### Email
- **Implementação:** [`EmailChannel`](market_alert/notifications/channels/email.py)
...
#### SMS
- **Implementação:** [`SMSChannel`](market_alert/notifications/channels/sms.py)
...
#### WhatsApp
- **Implementação:** [`WhatsAppChannel`](market_alert/notifications/channels/whatsapp.py)
...
#### Push / In-App
- **Implementação:** [`PushChannel`](market_alert/notifications/channels/push.py)
...
#### Slack
- **Implementação:** [`SlackChannel`](market_alert/notifications/channels/slack.py)
...
O projeto registra cada fase do *scraping* através do helper ``audit_scrape`` (`market_alert/utils/audit_logger.py`). Essa função grava arquivos
...
Em ``market_alert/tasks/metrics_tasks.py`` ficam as tarefas agendadas no `celery_app.conf.beat_schedule`:
