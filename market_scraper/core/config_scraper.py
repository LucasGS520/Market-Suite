...
class Settings(ConfigBase):
    ...
    SCRAPER_HEADERS_DEFAULT_COOKIES: str = os.getenv(
        ...
        "",
    )  # Cookies opcionais enviados por padrão (formato "chave=valor||...")
    SCRAPER_HEADERS_ADDITIONAL: str = os.getenv(
        "SCRAPER_HEADERS_ADDITIONAL",
        "",
    )  # Headers adicionais enviados além do conjunto mínimo ("Header=valor||...")
    ...
    SCRAPER_HTTP_DOMAIN_TIMEOUTS: str = os.getenv(
        ...
        "",
    )  # Permite sobrescrever timeout total por domínio ("dominio=segundos||...")
    ...
    def resolve_domain_timeout(self, domain: str | None, fallback: float) -> float:
        ...
        O formato aceito também utiliza ``||`` como separador e valores em
        segundos. Nomes de domínio são normalizados em minúsculas.
        """
