...
def test_user_agent_persists(monkeypatch):
    ...
    uas = iter(["UA1"])
    monkeypatch.setattr("alert_app.utils.user_agent_manager.random.choice", lambda lst: next(uas))
...
def test_user_agent_rotates_after_timeout(monkeypatch):
    ...
    times = [0.0, 1.0, 6.0]
    monkeypatch.setattr("alert_app.utils.user_agent_manager.time.monotonic", lambda: times.pop(0))
