...
def patch_robots(monkeypatch):
    ...
    monkeypatch.setattr("market_alert.utils.robots_txt.get_redis_client", lambda: fake_redis)
    ...
    def fake_get(*a, **k):
        class FakeResp:
            ...
            text = ""
        return FakeResp()
    monkeypatch.setattr("market_alert.utils.robots_txt.requests.get", fake_get)


@pytest.mark.integration
def test_scraper_persiste_dados_usando_playwright(db_session):
    """ Garante que o HTML vindo do Playwright é processado e persistido """
    ...
    #Substitui o método fetch_html e a criação do cliente para não abrir navegador
    ...
    with patch(
        "market_alert.utils.playwright_client.PlaywrightClient.fetch_html",
        new=AsyncMock(return_value=html)
    ) as fetch_mock, \
        patch(
            "market_alert.services.services_scraper_common.get_playwright_client",
            fake_client
        ), \
        patch(
            "market_alert.services.services_scraper_common.parser.looks_like_product_page",
            return_value=True
        ), \
        patch("market_alert.services.services_scraper_common.parser.parse_product_details", return_value={
            "name": "Produto Teste",
        ...
        result = scrape_monitored_product(
            db=db_session, url=payload.product_url, user_id=uuid4(), payload=payload
        )
        ...
        assert result == {"status": "success", "product_id": "pid"}
