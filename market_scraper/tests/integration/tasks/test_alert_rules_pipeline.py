...
from market_alert.tasks import alert_tasks
...
def test_alerts_logged_after_comparison(monkeypatch):
    ...
    monkeypatch.setattr("market_alert.tasks.compare_prices_tasks.redis_client", DummyRedis())
    monkeypatch.setattr("market_alert.tasks.compare_prices_tasks.SessionLocal", DummySession)
    monkeypatch.setattr("market_alert.tasks.alert_tasks.SessionLocal", lambda: DummySession())
    monkeypatch.setattr("market_alert.tasks.alert_tasks.get_monitored_product_by_id", lambda db, pid: SimpleNamespace(id=pid, name_identification="prod", user_id=user.id))
    ...
    class DummyChannel(NotificationChannel):
        async def send_async(self, u, subject, message) -> dict | None:
            ...
            return None
    monkeypatch.setattr("market_alert.notifications.manager.get_notification_manager", lambda: NotificationManager([DummyChannel()]))

    logs = []
    monkeypatch.setattr("market_alert.notifications.manager.create_notification_log", lambda db, user_id, channel, subject, message, alert_rule_id=None, alert_type=None, provider_metadata=None, success=True, error=None: logs.append((channel, subject)))
    rule = SimpleNamespace(id="r1", rule_type=AlertType.PRICE_TARGET, threshold_value=6, threshold_percent=None, enabled=True)
    monkeypatch.setattr("market_alert.notifications.manager.get_active_alert_rules_for_product", lambda *a, **k: [rule])
    monkeypatch.setattr("market_alert.notifications.manager.has_recent_duplicate_notification", lambda *a, **k: False)
    monkeypatch.setattr("market_alert.notifications.manager.update_last_notified", lambda *a, **k: None)
    ...
    monkeypatch.setattr("market_alert.tasks.compare_prices_tasks.run_price_comparison", fake_run)
