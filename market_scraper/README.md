...
Este documento descreve o fluxo de coleta de dados do serviço ``market_scraper`` utilizando de baixo custo baseadas em **JSON** e **HTML estático**.
...
### JSON Endpoint
As classes derivadas de ``JsonEndpointStrategy`` executam chamadas HTTP a APIs públicas. A estratégia da Shopee, por exemplo, extrai `shopid` e `itemid` da URL, chama o endpoint oficial `/api/v4/item/get` e formata o preço antes de devolver o resultado.
Os dados bem sucedidos são armazenados em cache para reutilização.

### HTML Estático
As estratégias herdadas de ``HtmlStaticStrategy`` realizam download do HTML com cabeçalhos stealth e tentam extrair os campos via blocos `JSON-LD` ou meta-tags. 
Quando essas fontes falham, regras específicas de cada domínio (Mercado Livre, Amazon, Shopee e Magalu) são aplicadas como fallback.
